...
//!
//! Provides an interface intended to output information in a human readable format during
//! `tvm` initialization.
//!
...
pub struct Message<'a> {
    /// The control settings of the [`Message`].
    control: ControlData<'a>,
    /// The data that was to be logged.
...
pub struct ControlData<'a> {
    /// The target/module at which the [`Message`] was logged.
    target: &'a str,
    /// The logging [`Level`] at which the [`Message`] was logged.
    level: Level,
}
...
impl AtomicLevelFilter {
    ...
    /// Converts the raw [`u8`] to its corresponding [`LevelFilter`].
    #[inline(always)]
