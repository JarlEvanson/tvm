...
impl fmt::Display for ParseGuidError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            Self::InvalidLength => "given GUID string has wrong length: expected 36 bytes".fmt(f),
...
macro_rules! guid {
    ($expr:tt) => {{
        let guid = $expr;
        match $crate::data_types::Guid::try_parse(&guid) {
            Ok(guid) => guid,
            Err($crate::data_types::ParseGuidError::InvalidLength) => {
                panic!("provided GUID is of an invalid length")
            }
            Err($crate::data_types::ParseGuidError::InvalidSeperator { c: _, position: _ }) => {
                panic!("given GUID has an invalid seperator character")
            }
            Err($crate::data_types::ParseGuidError::InvalidHexCharacter { c: _, position: _ }) => {
